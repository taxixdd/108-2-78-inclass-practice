...
entranceYear <- as.integer(entranceYear)
get_courses <- function(entranceYear, major){
  pick1<-str_detect(courseStructure$應修系級,paste0("(?<=",major,")[1]"),negate =FALSE)
  pick2 <- str_detect(courseStructure$應修系級,paste0("(?<=",major,")[2]"),negate = FALSE)
  pick3 <- str_detect(courseStructure$應修系級,paste0("(?<=",major,")[3]"),negate = FALSE)
  pick4 <- str_detect(courseStructure$應修系級,paste0("(?<=",major,")[4]"),negate = FALSE)
  pick_a <- str_detect(courseStructure$學年,entranceYear,negate = FALSE)
  ans <- list(
    year1 = courseStructure$科目名稱[pick1&pick_a],
    year2 = courseStructure$科目名稱[pick2&pick_a],
    year3 = courseStructure$科目名稱[pick3&pick_a],
    year4 = courseStructure$科目名稱[pick4&pick_a]
  )
  return(ans)
}
